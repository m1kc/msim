...
public class Server extends Thread {
    ...
    /**
     * Отправляет в ответ клиенту пакет с заданными типом и значением поля
     * content.
     */
    ...
    public void accountConnected(String account) throws IOException {
        for (Contact c : Storage.getContactsForIteration()) {
            if (c.ownerAccount.equals(account)) {
                for (Connection conn : connections) {
                    if (conn.account.equals(c.id)) {
            ...
            switch (p.type) {
                ...
                case "contacts-list":
                    ...
                    for (Contact q : Storage.getContactsForIteration()) {
                        if (q.ownerAccount.equals(c.account)) {
                            if (b.getGroup(q.group) == null) {
                                b.addGroup(q.group);
                            }
                ...
                case "contacts-rename":
                    ...
                    for (Contact q : Storage.getContactsForIteration()) {
                        if ((q.ownerAccount.equals(c.account)) && (q.id.equals(id))) {
                            q.nick = newNick;
                            success = true;
                        }
                    }
                ...
                case "contacts-remove":
                    up:
                    for (Contact q : Storage.getContactsForIteration()) {
                        if ((q.ownerAccount.equals(c.account)) && (q.id.equals(p.content))) {
                ...
                case "contacts-groups-rename":
                    ...
                    for (Contact q : Storage.getContactsForIteration()) {
                        if ((q.ownerAccount.equals(c.account)) && (q.group.equals(oldName))) {
                            q.group = newName;
                            success = true;
                        }
                    }
                ...
                case "contacts-groups-remove":
                    ...
                    for (Contact q : Storage.getContactsForIteration()) {
                        if ((q.ownerAccount.equals(c.account)) && (q.group.equals(p.content))) {
                            rm.add(q);
                        }
                    }
                    for (Contact q : rm) {
                ...
                case "presence-poll":
                    for (Contact q : Storage.getContactsForIteration()) {
                        ...
                        if (q.ownerAccount.equals(p.source)) {
                            if (isOnline(acc)) {
